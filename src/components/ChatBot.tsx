  ...
  const toggleHintModal = async () => {
    ...
    if (!isHintOpen) {
      ...
      try {
        ...
        setError(
          error.response?.data?.message || "NPC 조언을 받아오는데 실패했습니다."
        );
      } finally {
        setLoading(false);
      }
              ...
              {loading ? (
                ...
                <div className="space-y-4 animate-fadeIn">
                  {npcAdvice && npcAdvice.response && (
                    <>
                      {Object.entries(npcAdvice.response).map(
                        ([key, value], index) => (
                          <div
                            key={key}
                                  ...
                                  <div
                                    ...
                                    style={{
                                      ...
                                      backgroundColor: `rgb(${Math.round(
                                        255 * (1 - value.survival_rate / 100)
                                      )}, ${Math.round(
                                        255 * (value.survival_rate / 100)
                                      )}, 0)`,
                                    }}
                                  ></div>
                                </div>
